...
namespace Algorithm_206038929_314620071 {
    ...
    public:
        ObservedTank(size_t y, size_t x, int playerIndex, size_t shellCount,
                     std::optional<Direction> dir = std::nullopt) : ObservedEntity(y, x), playerIndex(playerIndex),
                                                                    shellCount(shellCount), assumedDirection(dir) {}

        ObservedTank(Coordinates coords, int playerIndex, size_t shellCount,
                     std::optional<Direction> dir = std::nullopt) : ObservedEntity(coords), playerIndex(playerIndex),
                                                                    shellCount(shellCount), assumedDirection(dir) {}
    ...
    private:
        ...
        bool firedLastTurn;
    public:
        inline explicit ObservedShell(Coordinates coords, std::optional<Direction> assumedDirection = std::nullopt,
                                      bool firedLastTurn = false) : ObservedEntity(coords),
                                                                    assumedDirection(assumedDirection),
                                                                    firedLastTurn(firedLastTurn) {}

        inline ObservedShell(size_t y, size_t x, std::optional<Direction> assumedDirection = std::nullopt,
                             bool firedLastTurn = false) : ObservedEntity(y, x), assumedDirection(assumedDirection),
                                                           firedLastTurn(firedLastTurn) {}

...
namespace Algorithm_206038929_314620071 {
    SnipingTank::SnipingTank() = default;
...
// Dodge if needs to -> rotate towards target if possible -> shoot if possible -> Get info
    ActionRequest SnipingTank::getAction(const FullBattleInfo &battleInfo) {
        ...
        bool targetIsInDirection = areColinear(myCoords, targetCoords.value(), battleInfo.getRows(),
                                               battleInfo.getCols());

        if (myTank.canFire() && targetIsInDirection)
            return ActionRequest::Shoot;
        Coordinates coordsInFront = DirectionUtils::nextCoordinate(myTank.getDirection().value(), myCoords,
                                                                   battleInfo.getRows(), battleInfo.getCols());

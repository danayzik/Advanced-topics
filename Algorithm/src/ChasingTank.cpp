...
namespace Algorithm_206038929_314620071{
    ChasingTank::ChasingTank(const FullBattleInfo &battleInfo) : gridGraph(
            std::vector<std::vector<HeapNode>>(battleInfo.getRows(), std::vector<HeapNode>(battleInfo.getCols()))) {}
    ...
    ActionRequest ChasingTank::getAction(const FullBattleInfo &battleInfo) {
        ...
        bool shouldFire =
                gridGraph[currY][currX].totalCost == INT32_MAX || (!friendlyInRange && dist < 4 && myTank.canFire() &&
